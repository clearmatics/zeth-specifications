...
\nomenclature[F]{$\bnFieldBitCap$}{Field capacity of $\FFx{\rBN}$. It is defined as the maximum bit length $l$ such that all numbers $x$ encoded on $l$ bits are elements of $\FFx{\rBN}$. In other words, $\bnFieldBitCap = \max_{x \in \FFx{\rBN}} \smallset{\ceil{\log_2{x}}} \suchthat \sum_{i \in [\bnFieldBitCap]} 2^i \in \FFx{\rBN}$ \nomunit{$\floor{\log_2{\rBN}} = 253\, bits$}}
