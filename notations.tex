...
\nomenclature[A, 041]{$S \subseteq T$}{$S$ is a subset of $T$, i.e.~$\forall x \in S \implies x \in T$}
\nomenclature[A, 042]{$S \subset T$}{$S$ is a \emph{proper} (or ``strict'') subset of $T$, i.e.~$\forall x \in S \implies x \in T \land \exists y \in T, y\not\in S$}
...
\nomenclature[A, 046]{$f \colon S \to T$}{Function $f$ that maps items from the non-empty set $S$, called ``the domain'', to the non-empty set $T$, called ``the co-domain''}
...
\nomenclature[A, 110]{$S^{n}$}{n-fold Cartesian product of $S$ with itself, i.e.~$S^{n} = \{(x_1, \ldots, x_n) : \forall i \in \smallset{1, \ldots, n}, x_i \in S\}$, where $n \in \NN$}
...
\nomenclature[A, 170]{$x \concat y$}{Infix notation for the concatenation function, $\concat \colon \Lambda^{*} \times \Lambda^{*} \to \Lambda^{*}$. If $\len{x}= n, \len{y} = m: (n, m) \in \NN^2$, then $\len{z} = n + m, z = x \concat y$}
\nomenclature[A, 180]{$\trunc{k}{x}$}{$\trunc{}{} \colon \Lambda^{*} \to \Lambda^{k}$ is the truncation function that returns the sequence formed from the the first $k$ elements of $x$, where $x \in \Lambda^{*}$. If $k > \len{x}$, then $\trunc{k}{x} = x$}
...
\nomenclature[A, 210]{$\append{l}{x}$}{$\append{}{} \colon D^{n} \times D \to D^{n+1}$ is the algorithm that appends $x$ to the list of $n$ element(s) $l$, if all $x$ and $l$ share the same datatype $D$}
...
\nomenclature[A, 260]{$\FFx{q}$}{Finite field of cardinality $q = p^m$, where $p$ is prime}
...
% Zeth notations - D
\nomenclature[D, 010]{$\zkp$}{Zero-knowledge non-interactive argument of knowledge (zk-snark for short). \zkp~is also informally referred to as a ``zk-proof'', or simply ``proof'' when clear from context}
...
\nomenclature[F]{$\bnFieldBitCap$}{Field capacity of $\FFx{\rBN}$. It is defined as the maximum bit length $l$ such that all numbers $x \in \BB^l$ are field elements $x \in \FFx{\rBN}$: $\max_n \smallset{ \forall x \in \bin^n,\ x \in \FFx{\rBN}}$ \nomunit{$\floor{\log_2{\rBN}} = 253\, bits$}}
\nomenclature[F]{$\jsin, \jsout, \jsmax$}{The number of inputs and outputs of a joinsplit and $\jsmax = \max(\jsin, \jsout)$}
...
\nomenclature[F]{$\txDefaultGas$}{The intrinsic gas cost of an \ethereum~transaction \nomunit{$21 000\, \wei$}}
\nomenclature[F]{$\ethWordLen$}{Width of a storage cell on the Ethereum Virtual Machine stack, i.e.~size of a word on the EVM \nomunit{$256\, bits$}}
\nomenclature[F]{$\pSecp$}{Prime defining the finite field of curve $\secpCurve$~\cite{secp256k1}}
\nomenclature[F]{$\secpFieldBitLen$}{Bit-length of a field element $x \in \FFx{\pSecp}$ \nomunit{$\ceil{\log_2{\pSecp}} = 256\, bits$}}
...
\nomenclature[F]{$\encZethNoteLen$}{Size of an encrypted note \nomunit{$1216\, bits$}}
