...
By assuming that the different rounds of the scheme are independent, the probability that an input difference gets mapped to an output difference becomes bounded as $\Pr[F(x + d) - F(x) = D] \leq ((\exponent{}-1)/p)^\rounds$ .
