...
To preserve the soundness of \zeth, and make sure that no $\zparty{U}$ is able to create value by double spending $\zethnotes$, various checks need to satisfied. The function $\zethVerifyTx$ is defined as the function that returns $\true$ if all the checks are satisfied, and $\false$ otherwise.
...
\begin{enumerate}
    \item Check that all the values of the primary inputs' ($\inp{\mix}.\primInp$) entries are elements of the scalar field over which the zk-proof is generated:
        ...
        \]
    \item Unpack the nullifiers, signature hash and public values:
        \begin{align*}
    ...
    \item Check the validity of the $\zethTx$ object ($\zethVerifyTx$):
        \begin{itemize}
            \item Check that $\inp{\mix}.\primInp.\hsig$ is correctly computed, i.e.~check that he following equation holds (to prevent transaction malleability, see:~\cref{appendix:trnm}):
                \[
                    \hsig = \crhhsig{\inp{\mix}.\primInp.\nfs{}, \inp{\mix}.\otsvk}
                \]
            \item Check that $\zkp$ is a valid zk-snark for $\inp{\mix}.\primInp$, i.e.~check that:
                \[
                    \zksnark.\verifier(\pparams_{\zksnark}, \zkp, \inp{\mix}.\primInp) = true
                \]
            ...
            \item Check that $\inp{\mix}.\otssig$ is a valid signature of the Ethereum sender's address $\addr$ (see:~\cref{zeth-protocol:create-tx}) and the attributes of $\inp{\mix}$, to prevent transaction malleability (see:~\cref{appendix:trnm}), i.e.~check that:
                \begin{align*}
                    ...
                    & where \ \msg = \crhots{\addr \concat \inp{\mix}.\primInp \concat \inp{\mix}.\zkp \concat \inp{\mix}.\ciphers}
                \end{align*}
            \item Check that $\inp{\mix}.\primInp.\mkroot$ corresponds to a valid state of the Merkle tree held on $\mixer$, i.e.~check that:
                ...
                \]
                where $\rootset'$ is the set of all Merkle roots corresponding to one of the state of the Merkle tree.
